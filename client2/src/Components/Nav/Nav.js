					...
					<div>
						...
						>
							<div style={{ marginTop: 40, marginBottom: 40 }}>
								<FontAwesomeIcon
									icon={faSadTear}
									style={{ fontSize: 60, color: "deeppink" }}
								/>
								<FontAwesomeIcon
									icon={faSadTear}
									style={{ fontSize: 60, color: "rgb(321,127,33)" }}
								/>
								<FontAwesomeIcon
									icon={faSadTear}
									style={{ fontSize: 60, color: "rgb(100,211,0)" }}
								/>
							</div>
							<h5 className="my-2 text-secondary">
								Sorry {this.props.user.name} time is up!!!
							</h5>
							<p className="text-secondary" style={{ fontSize: 20 }}>
								We will have to log you out automatically
							</p>
		...
		return (
			<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
				<span class="navbar-brand text-uppercase">
					<FontAwesomeIcon icon={faUser} /> Writer's name:{" "}
					...
					<ul class="navbar-nav">
						<li class="nav-item active">
							<span class="navbar-brand" href="#">
								<FontAwesomeIcon icon={faBookOpen} /> COURSE:{" "}
								{this.props.course}
							</span>
						</li>
					</ul>
				...
				<form class="form-inline">
					<span
						style={{
							color: ` ${minute < 2 && "red"}`,
							fontWeight: ` ${minute < 2 && 900}`,
						}}
						class={`time mr-4 ${minute < 2 ? "animate" : null}`}
					>
						<FontAwesomeIcon
						...
						/>{" "}
						{!this.state.finish
							? `TIME LEFT:  ${minute}MIN: ${seconds}SEC`
							: `TIMES'S UP:  ${0}MIN: ${0}SEC`}{" "}
					</span>
					<button
					...
					>
						{" "}
						SUBMIT <FontAwesomeIcon icon={faArrowCircleRight} />
					</button>
				</form>
			</nav>
		);
